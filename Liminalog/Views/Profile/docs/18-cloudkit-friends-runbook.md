...
10. Both sides receive the other's shared snapshot, each `FriendShareSnapshot.targetUserRecordName` matches the recipient account's CloudKit user record name, and each `CKShare` has `publicPermission = none` with the target account as a read-only participant after both initial creation and a later update.
