...
enum PreviewSupport {
    static let container: ModelContainer = {
        let schema = Schema([Category.self, CategorySet.self, Chapter.self, PlanBlock.self, UnlockItem.self, VisibilityPreset.self, UserSettings.self, Friend.self, FriendSet.self, FriendCategoryMapping.self, DailyCardSnapshot.self, DailyScoreSnapshot.self, CalendarEventCache.self])
    ...
    private static func seed(in context: ModelContext) {
        let categories = [
            ...
            Category(name: "自由時間", colorHex: "#F2C94C", icon: "gamecontroller.fill", sortOrder: 6, isDefault: true),
            Category(name: "家事", colorHex: "#56CCF2", icon: "house.fill", sortOrder: 7, isDefault: true)
        ]

        categories.forEach(context.insert)
        let weekdaySlots: [UUID?] = categories.map { Optional($0.id) }
        let holidayLayout: [UUID?] = [
            categories[4].id,
            categories[6].id,
            categories[5].id,
            categories[3].id,
        ...
        context.insert(CategorySet(name: "平日", sortOrder: 0, slots: weekdaySlots, isDefault: true))
        context.insert(CategorySet(name: "休日", sortOrder: 1, slots: holidayLayout, isDefault: true))
        ...
        let samples: [(Int, Int, Category, String?, String?, String?, Bool)] = [
            (7, 90, categories[4], nil, "😴", nil, false),
            (9, 75, categories[0], "英語の復習", "💪", "図書館", true),
            (11, 45, categories[2], nil, nil, "電車", true),
            (13, 120, categories[1], "UIを整理", "🔥", nil, true),
            (16, 30, categories[3], "コーヒー", "☕️", "カフェ", false),
        ]
        ...
        let plannedSamples: [(Int, Int, Int, Category, String)] = [
            (0, 0, 420, categories[4], "睡眠"),
            (7, 0, 60, categories[7], "朝の準備"),
            (8, 0, 60, categories[2], "移動"),
            (9, 0, 120, categories[0], "英語と課題"),
            (11, 0, 45, categories[2], "移動"),
            (11, 45, 75, categories[6], "昼休み"),
            (13, 0, 180, categories[1], "制作作業"),
            (16, 0, 30, categories[3], "休憩"),
            (16, 30, 90, categories[0], "復習"),
            (18, 0, 60, categories[2], "帰宅"),
            (19, 0, 150, categories[5], "自由時間"),
            (21, 30, 60, categories[3], "夜の休憩"),
            (22, 30, 90, categories[4], "睡眠"),
        ]
        ...
        if let activeStart = calendar.date(byAdding: .hour, value: 18, to: startOfDay) {
            let active = Chapter(category: categories[5], startTime: activeStart)
            active.note = "アプリの試作"
            active.mood = "🤩"
        ...
        let mockSchedules: [(Int, Int, Int, Category, String?, String?, String?, Bool)] = [
            (2, 8, 45, categories[2], "通勤", nil, "電車", true),
            (2, 10, 90, categories[1], "定例ミーティング", "📝", "オフィス", true),
            (2, 13, 60, categories[0], "資料読み込み", nil, nil, false),
            (2, 16, 45, categories[3], "散歩", "🌿", "公園", true),
            (2, 20, 75, categories[5], "映画", "🎬", nil, true),
            (5, 9, 120, categories[1], "企画レビュー", "🔥", "会議室A", true),
            (5, 14, 90, categories[0], "SwiftUI実験", "💡", nil, true),
            (8, 7, 60, categories[6], "朝カフェ", "☕️", "駅前", false),
            (8, 19, 150, categories[6], "友達とごはん", "🍜", "渋谷", true),
            (11, 10, 240, categories[1], "集中作業", "💻", nil, false),
            (14, 9, 45, categories[2], "移動", nil, "新幹線", true),
            (14, 11, 180, categories[0], "読書会", "📚", "図書館", true),
            (14, 18, 60, categories[3], "休憩", nil, nil, false),
            (18, 6, 120, categories[4], "二度寝", "😴", nil, false),
            (18, 12, 75, categories[6], "ランチMTG", nil, "カフェ", true),
            (21, 8, 30, categories[2], "移動", nil, nil, true),
            (21, 9, 180, categories[1], "実装", "🔥", nil, false),
            (21, 13, 60, categories[6], "昼休み", "🍱", nil, true),
            (21, 15, 120, categories[0], "設計メモ", "✍️", nil, true),
            (21, 19, 90, categories[5], "音楽", "🎧", nil, true),
            (24, 10, 90, categories[0], "日本語カレンダー確認", "✅", nil, true),
            (27, 13, 180, categories[1], "週次まとめ", "📊", "自宅", false),
            (30, 9, 60, categories[6], "モーニング", nil, "喫茶店", true),
            (30, 20, 120, categories[5], "ゲーム", "🎮", nil, true),
        ]
