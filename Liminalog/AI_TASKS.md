...
- [x] **G.（軽微）リング中央の整理**：グリフ＋pt＋ラベルで詰まり気味。docs/13 §5.3「中央は余白基調」へ。<!-- 担当: Codex, 完了: 2026-06-02。中央の大きなグリフ玉を小バッジ化してリング上側へ逃がし、中央はスコア数字を主役にしてラベルを極小化 -->
...
- [~] `primary` / `reward` / `dawn` / `dusk` の曙用hexを詰め直し、黄土色化・ラベンダーと黄の温度割れ・彩度段差を潰す <!-- 担当: Codex, 進捗: 2026-06-02。primary/duskは白背景へ馴染む淡い紫へ、rewardは昨日カードのライトとして使える柔らかい金へ調整。`liminalAccentLight(in:)` を追加し、曙だけDailyReflectionCardのmetrics/fact周辺に朝ライトを差す。リング中央は紫グローから暖色ベースの金〜ピーチ系グローへ切り替え、薄ラベンダーは外枠/チップの語彙として維持 -->
