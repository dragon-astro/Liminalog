...
- [ ] 友達共有スケーラブル化: CloudKit輸送層の個別レコード化＋差分同期（docs/20 §7.3-2,3。SharedPlan/SharedChapter 1件1レコードupsert/delete、ゾーン変更トークン差分受信。受信反映は `FriendSharedRecordReconcilePolicy`/`FriendSharedRecordStore` を流用） <!-- 担当: Codex, 理由: CloudKit/SwiftDataコンテナ不変条件＋並行制御。二台実機検証必須・友達正式リリース更新に同梱（docs/20 §5） -->
- [ ] 友達共有スケーラブル化: 塊JSON（sharedPlansJSON/sharedActivitiesJSON）と二重書きの撤去（docs/20 §7.3-4） <!-- 担当: 未定, 依存: 上2タスク完了後 -->
