...
- [x] 友達共有スケーラブル化: UIを個別行の範囲クエリへ移行（docs/20 §7.3-1） <!-- 担当: Claude, 完了: 2026-06-10。メインカレンダーオーバーレイ/FriendCalendarView/デイビュー/検索シートを置換。FriendShareSnapshotCache の撤去は塊JSON撤去（§7.3-4）と同時 -->
- [x] 友達共有スケーラブル化: CloudKit輸送層の個別レコード化＋差分同期（docs/20 §7.3-2,3） <!-- 担当: Claude（Codexトークン切れのため代行）, 完了: 2026-06-11。設計メモ=docs/20 §8 -->
- [x] 友達共有スケーラブル化: 塊JSONと二重書きの撤去（docs/20 §7.3-4） <!-- 担当: Claude, 完了: 2026-06-11 -->
- [ ] シードの決定的UUID化: 組み込みデフォルト（Category/CategorySet/VisibilityPreset）のIDを固定文字列由来の安定UUIDにし、再シードでも同一IDが生成されるようにする（CloudKit重複統合が原理的に不要になる堅牢化） <!-- 担当: 未定, 理由: CloudDuplicateMergeStore は残すが防御層に格下げできる -->
- [!] 友達共有スケーラブル化: **二台実機での反復検証**（共有・差分・削除・可視性・トークン失効。docs/20 §8.3 の未検証5項目） <!-- 担当: Codex+人間, 進捗: 2026-06-12 に実機2台E2E run codex-20260612D で初回共有・差分更新/追加/削除・公開範囲OFF相当の赤字化/tombstone差分・cleanupに加え、削除済みshared itemをrecipientのshared zone full fetch（tokenなし）+ reconcileでローカルから回収できることを実CloudKitで確認。run D: 初回accept/fetch 90.54秒、差分取得 2.52秒、redaction/tombstone差分 2.10秒、full fetch reconcile 86.65秒。run E も iPhone 13=recipient / iPhone 16=owner でcleanupまで再実行し、初回accept/fetch 91.52秒、差分取得 2.28秒、redaction/tombstone差分 1.31秒、full fetch reconcile 88.07秒で成功。run G では保存済みtokenを明示的に破棄し、1回のfull fetchでreplacement tokenを保存し直す復旧パスまで確認。初回accept/fetch 89.51秒、差分取得 1.72秒、redaction/tombstone差分 2.26秒、token破棄→full fetch→token再保存→reconcile 87.62秒でcleanupまで成功。iPhone 13実機bulk smokeで360 child recordsのpublish 5.20秒/delete 12.45秒/全体19.38秒を確認。表示キャッシュは3,600行からカレンダー範囲42 plans+42 chaptersを0.016秒で取得する回帰テストに加え、友達カレンダー月ページ生成を同じ3,600行級LocalCacheから測る `FriendCalendarPageDataBuilder` 回帰テストを追加。iPhone 13実機で範囲クエリ0.0027秒・月ページ生成0.0101秒、iPhone 16実機で範囲クエリ0.0018秒・月ページ生成0.0058秒を確認。token失効後のfull fetch/reconcileで欠落行を回収するユニットテストと、フルリロード通知が対象友達の全カレンダーページ無効化になる回帰テストを追加。共有URL失効/ブロック/同意喪失で行キャッシュを消すパスとFriends画面の手動クリア経路も full reload 通知を投げるよう修正。`FriendCalendarCacheInvalidationPolicy` で画面キャッシュ無効化判断を切り出し、通常カレンダーの友達overlay更新も同policyへ統一。通知境界/ヘルパー/画面キャッシュpolicyテスト9件を simulator と iPhone 16実機で確認（同日iPhone 13はCoreDevice接続断でテスト起動前失敗）。その後、アンカー月から離れた表示月でも現在オフセット基準で再補充するテストを追加し、policy統一後のiPhone 16実機で同10件成功、Release buildも再成功。新ビルド初回起動時のストア準備fetch失敗に備え、アプリ本体の初回bootstrapだけ短いreadiness retryを追加し、`BootstrapStoreTests` 3件をsimulator、bootstrap+友達差分キャッシュ計12件をiPhone 16実機で確認。Release buildも成功。devicectlでiPhone 13/16両方のアプリ起動とプロセス生存を確認。実機E2E中にrecipient marker権限とSwiftData差分削除反映の問題を検出/修正。残りは友達カレンダーの手動UI確認、意図的changeTokenExpiredの実CloudKit再現 -->
...
- [x] 先月比差分バー <!-- 担当: Codex, 完了: 2026-06-06。`DashboardPeriodDeltaCard` を期間対応にし、月間は先月比、年間は前年比として表示。月間/年間のデフォルトカード順にも前期間比を追加 -->
...
- [x] **F. 昨日ページの重複撤去**：カード下の旧 `YesterdayCategoryBreakdown`＋`YesterdayInsightCard` を、カードの**展開層（2層の裏）に畳む or 撤去**（docs/12 §3）。<!-- 担当: Codex, 完了: 2026-06-02。カード下の旧カテゴリ内訳/インサイトブロックと未使用View群を撤去し、DailyReflectionCardを昨日ページの主役に整理 -->
- [-] **G.（軽微）リング中央の整理**：グリフ＋pt＋ラベルで詰まり気味。docs/13 §5.3「中央は余白基調」へ。<!-- 撤回: 2026-06-02 ユーザー確認で「修正前の中央構成の方が良い」と判断。中央の大きなグリフ+pt構成へ戻し、説明ラベルとライトの黄色グローは抑制 -->

- [x] **StatsEngine / PatternDetector**（統計タブと共有の集計層・二重集計回避）<!-- 担当: Codex, 完了: 2026-06-02。`DailyCardPatternDetector` を Logic 層へ移動し、`StatsEngine` から DailyCard / Dashboard の日別ScoreSummary・時間帯・前期間差分を共通生成。DailyCardEngine はコピー選択だけを保持し、統計タブとの二重集計を回避 -->
...
- [~] **検出器カタログ実装**（docs/12 §5.1 中立12型）＋ 選抜ロジック（§5.2 パンチ×逸脱・型重複回避・床型保険）<!-- 担当: Codex, 2026-06-02進捗: A/B/C/G/I/L と床型保険・昨日と同型回避を `DailyCardPatternDetector` の候補選抜へ接続。初記録・久々・自己最長・いつもより増減・宣言ベース・カテゴリ構成・記録連続を punch/newsworthiness で選抜し、床型はパンチ候補がない日にfactへ出す。前日spotlightと同じkindは別候補がある場合に回避。D/F は既存の形称号/予定一致で継続。H体感換算・J気分・K友達・手書き換算表は後続 -->
...
- [x] カードスナップショット保持（過去日カード再描画・持続タイプ・称号コレクション）→ docs/04 追補 <!-- 担当: Codex, 完了: 2026-06-02。`DailyCardSnapshot` / `DailyCardSnapshotStore` を追加し、昨日ページ表示時に `DailyPersona`・fact・カテゴリ上位を日付単位で upsert。`DailyCardPersonaKind` で持続タイプを保持し、保存済みsnapshotから称号コレクションを集計可能にした。CloudKit互換のため fact/category はJSON文字列保持。docs/04追補とテスト追加 -->
...
- [x] **P2-4 自己説明ラベル削除**（§1.3：「24時間バー」見出し等）。<!-- 担当: Codex, 完了: 2026-06-02。TimelineView / SharedTimelineReadOnlyView の DayOverviewBar から「24時間バー」見出しを撤去。追加監査で DailyReflectionCard の「予定と実績/実績のみ」ピルをアイコン化、CalendarDayView の「予定と実績」見出しと友達日詳細の「共有された予定と実績」サブタイトルを削除。説明は必要箇所だけ accessibilityLabel に残した -->
- [x] **P2-5 今日ページの 実績/予定 segmented を控えめ inline トグルへ・リスト=実績デフォルト**（§2）。<!-- 担当: Codex, 完了: 2026-06-02。TimelineView と SharedTimelineReadOnlyView の segmented Picker を下線付き inline toggle に降格。中央寄せ + 適度な余白で元トグルの重心は維持。TimelineTab.actual が先頭かつ保存値defaultも actual のためリストは実績デフォルトを維持 -->
...
- [x] **P2-7 昨日ページ重複ブロック撤去**（§3＝§7.7.2 F）。<!-- 担当: Codex, 完了: 2026-06-02。`YesterdayCategoryBreakdown` / `YesterdayInsightCard` と関連未使用Viewを撤去し、昨日ページはDailyReflectionCard単体を中心に整理 -->
- [ ] **P3-8 色の希少・chrome/data分離の全画面洗い出し**（§1.2）。<!-- 担当: Claude -->
- [x] **P3-9 友達一覧の主役をステータスリストに・ランキング従・@handle非強調**（§7）。<!-- 担当: Codex, 完了: 2026-06-02。ランキングは友達数が増えても長くならない固定サマリーとして上部に置き、カードのサイズ/面/スコア文字を控えめにして従に調整。「今の友達」リストはその下で名前+現在ステータス+ひとことを主役にし、@handleは一覧主情報に出さない -->
...
- [~] `primary` / `reward` / `dawn` / `dusk` の曙用hexを詰め直し、黄土色化・ラベンダーと黄の温度割れ・彩度段差を潰す <!-- 担当: Codex, 進捗: 2026-06-02。primary/duskは白背景へ馴染む淡い紫へ、rewardは昨日カードのライトとして使える柔らかい金へ調整。`liminalAccentLight(in:)` を追加し、曙だけDailyReflectionCardのmetrics/fact周辺に朝ライトを差す。リング中央は暖色ライトの主張が白地で浮いたため、黄を抜いた薄ラベンダーの低密度グローへ再調整 -->
...
- [x] Siri ショートカット拡充（App Intents）<!-- 担当: Codex, 完了: 2026-06-02。`LiminalogAppShortcutsProvider` を追加し、Today/カレンダー/統計/プロフィールを開く4ショートカットと、現在の記録を閉じる `EndChapterIntent` を登録。画面遷移は App Group の `shortcut.pendingRoute` を `RootTabView` が起動/復帰時に消費する。カテゴリ指定の `StartChapterIntent` はWidget/Live Activity向けとして継続 -->
...
- [!] **【Codex本題】友達共有を個別レコード化（全履歴×軽量）**。塊JSON方式は CloudKit 1MB上限・メモリ・カレンダー描画で破綻するため、`SharedChapter`/`SharedPlan` を1件1レコード＋ゾーン変更トークン差分同期＋受信側ローカル個別行＋表示は範囲ロード、へ作り直す。設計：**[../Liminalog/Views/Profile/docs/20-friend-share-scalable.md](../Liminalog/Views/Profile/docs/20-friend-share-scalable.md)**。2026-06-12 の二台実機E2E run `codex-20260612D/E/G` で初回共有、差分更新/追加/削除、公開範囲OFF相当の赤字化/tombstone差分、削除済みshared itemのfull fetch/reconcile回収、cleanup まで確認済み。run G は初回accept/fetch 89.51秒、差分取得 1.72秒、redaction/tombstone差分 2.26秒、token破棄→1回full fetch→replacement token保存→reconcile 87.62秒。通常差分は1〜2秒台、初回共有とtoken喪失後の復旧full fetchは90秒級。iPhone 13実機bulk smokeで360 child recordsのpublish 5.20秒/delete 12.45秒/全体19.38秒を確認。表示キャッシュは3,600行からカレンダー範囲42 plans+42 chaptersを0.016秒で取得する回帰テスト追加済み。token失効後のfull fetch/reconcileで欠落行を回収し、フルリロード通知が対象友達の全カレンダーページ無効化になるユニットテスト追加済み。Gate23で保存済みtokenが`changeTokenExpired`相当のエラーになった時にnil tokenのfull fetchへ落ちるapp側fallbackもテスト化し、Simulator/iPhone 16/iPhone 13で成功。通常カレンダーの友達overlay更新も同じ画面キャッシュpolicyへ統一し、iPhone 16実機でpolicyテスト10件とRelease buildが成功。devicectlでiPhone 13/16両方の起動とプロセス生存を確認。残: 友達カレンダーの手動UI確認、意図的changeTokenExpiredの実CloudKit再現。<!-- 担当: Codex -->
  - 2026-06-12 Codex追加: `CloudKitCurrentUserRecordResolver` を追加し、`CloudKitSocialStore` / `CloudFriendShareStore` の現在ユーザーID取得をプロセス内キャッシュ化。`Notification.Name.CKAccountChanged` で無効化するため、アカウント切替時も次回取得へ戻る。iPhone 16実機で resolver tests 3件、実CloudKit smoke 1件（3.297秒）、Release build Gate12 が成功。
  - 2026-06-12 Codex追加: `FriendsView` の自動友達申請refreshを `CloudFriendLocalStatePolicy` でスロットル。同一ユーザーの初回表示refreshは一度だけ、失敗直後の再表示は120秒抑制、push通知/手動更新は即時。iPhone 16実機で `CloudFriendLocalStatePolicyTests` 9件、Release build Gate13 が成功。
  - 2026-06-12 Codex追加: アプリ起動/復帰時の送信側full publishを10分クールダウンし、予定/実績保存など明示refreshは `cloudFriendShare.pendingOutgoingRefresh` マーカーで次回起動/復帰でも拾うようにした。これにより復帰だけで全履歴fetch + CloudKit差分照合が連発しない。iPhone 13実機で `CloudFriendLocalStatePolicyTests` 12件、実CloudKit smoke 1件（1.989秒）、Release build Gate16、`git diff --check` が成功。
  - 2026-06-12 Codex追加: 友達カレンダー表示側のキャッシュ無効化と範囲ロードをiPhone 13/16実機で再確認。さらに `FriendCalendarPageDataBuilder` を切り出し、画面とテストが同じ月ページ生成経路を見るようにした。iPhone 13実機で `FriendSharedRecordChangeImpactTests` 10件、`FriendSharedRecordStoreTests` 10件が成功し、大容量範囲クエリは3,600行から42 plans + 42 chaptersを0.0027秒、月ページ生成は35 important plansを0.0101秒。iPhone 16実機でも同20件が成功し、範囲クエリ0.0018秒、月ページ生成0.0058秒。
  - 2026-06-12 Codex追加: zone changes の本番適用処理を `FriendShareZoneChangeApplier` に切り出し、`CloudFriendShareRefreshCoordinator` から同じ経路を呼ぶ形にした。fullZone fetch は reconcile で欠落行を削除し root snapshot も反映、incremental root change はfull reloadにせず今日/昨日範囲だけ通知する。iPhone 16 / iPhone 13 実機で `FriendShareZoneChangeApplierTests` 2件が成功。
  - 2026-06-12 Codex追加: 最新差分後の実CloudKit smoke Gate22。iPhone 16実機で `testRealDeviceCanReachCloudKitFriendInfrastructure` 2.04秒、`testSharedItemRecordsRoundTripOnRealCloudKit` 3.98秒。iPhone 13実機で同2.58秒 / 3.72秒。両端末ともCloudKit到達とparent付きSharedPlan/SharedChapter保存削除が成功。記録更新後のRelease build Gate22も成功。
  - 2026-06-12 Codex追加: token失効fallbackを `FriendShareZoneChangeFetchRecoveryPolicy` に切り出し、保存済みtokenで `changeTokenExpired` 相当のエラーを受けた時にnil tokenで1回だけfull fetchへ落ちること、非token系CKErrorではretryしないことをテスト化。`FriendShareZoneChangeFetchRecoveryPolicyTests` 3件が Simulator / iPhone 16実機 / iPhone 13実機で成功。実CloudKitサーバー由来の意図的 `changeTokenExpired` 再現は未実施。
...
- ✅ **`currentUserRecordName()`/`fetchCurrentUserRecordID()` の多重呼び出しを削減**：`CloudKitCurrentUserRecordResolver` で `accountStatus()`＋`fetchUserRecordID()` を同一プロセス内キャッシュ化し、`Notification.Name.CKAccountChanged` で破棄する。`CloudKitSocialStore` と `CloudFriendShareStore` は同じ resolver を共有。検証: iPhone 16実機 `CloudKitCurrentUserRecordResolverTests` 3件、`CloudKitLiveDeviceSmokeTests/testRealDeviceCanReachCloudKitFriendInfrastructure` 1件（3.297秒）、Release build Gate12 成功。
- ✅ `FriendsView.task` の `refreshCloudRequestsIfPossible()` をスロットル化。自動refreshは同一ユーザーで一度だけ、失敗直後の再表示は120秒抑制、`CloudKitFriendEventBridge.friendConsentDidChange` と手動更新は即時にした。検証: iPhone 16実機 `CloudFriendLocalStatePolicyTests` 9件、Release build Gate13 成功。
- ✅ `LiminalogApp` の lifecycle refresh が起動/復帰のたびに送信側full publishを走らせる問題を緩和。10分クールダウン＋明示変更pendingマーカーで、通常復帰は軽くしつつ未送信変更は次回起動/復帰で拾う。検証: iPhone 13実機 `CloudFriendLocalStatePolicyTests` 12件、実CloudKit smoke 1件（1.989秒）、Release build Gate16 成功。
- ✅ 友達カレンダー表示側の重さを再確認。表示範囲だけをLocalCacheから読む `FriendSharedRecordStore`、月ページ生成の `FriendCalendarPageDataBuilder`、対象月だけを無効化する `FriendCalendarCacheInvalidationPolicy` を iPhone 13/16 実機で確認。検証: iPhone 13は関連20件成功、3,600行から42日分取得0.0027秒・月ページ生成0.0101秒。iPhone 16も関連20件成功、42日分取得0.0018秒・月ページ生成0.0058秒。
- ✅ token喪失/full fetch復旧後の本番適用経路を直接テスト可能化。`FriendShareZoneChangeApplier` が fullZone fetch を reconcile として扱い、欠落行削除・root snapshot反映・必要な画面通知を返す。検証: iPhone 16 / iPhone 13 実機 `FriendShareZoneChangeApplierTests` 2件成功。
- ✅ `changeTokenExpired` fallback分岐を直接テスト可能化。`FriendShareZoneChangeFetchRecoveryPolicy` が保存済みtoken失効時だけnil token full fetchへretryし、非token系CKErrorはretryしない。検証: `FriendShareZoneChangeFetchRecoveryPolicyTests` 3件が Simulator / iPhone 16実機 / iPhone 13実機で成功。
- ✅ 最新差分後も実CloudKit基盤が両端末で通ることを再確認。検証: Gate22 iPhone 16実機 smoke 2件成功（CloudKit到達2.04秒、parent付き子レコード保存削除3.98秒）、iPhone 13実機 smoke 2件成功（2.58秒、3.72秒）、Release build Gate22 成功。
...
| 日付 | 担当 | 内容 |
|---|---|---|
| 2026-06-11 | Claude | 「友達タブを開くとアプリ全体が重くなる」を修正。原因: FriendsView の**全期間Chapter/PlanBlockの常駐@Query**。TabViewはタブのビューを生かし続けるため、一度開くと以降あらゆる記録保存でこの全件クエリが再評価されていた（PlanBlock側は段階3以降デッドコードだった）。対策: planBlocksクエリ削除・chaptersクエリを公開時のみのオンデマンドfetchへ。検証: テスト256件全パス、iPhone 13で配備・クリーン起動確認。※友達カレンダーが空の件は、本日の一連のクリーンアップで両アカウントの実データ（Chapter/PlanBlock）が消えたため**正常動作**の可能性が高い — iPhone 16側でID再確定+データ作成+公開が走った後にE2E再確認（16が空き次第）。 |
| 2026-06-11 | Claude | **データ増殖・ストア破壊の真犯人を特定・修正**: ウィジェット拡張が開発ストアリセット機構の独自コピー（バージョン定数 2026060501=2世代前）を持ち、本体（2026061101）とdefaultsキーを取り合って**ウィジェット更新のたびにストア全削除→本体が再シード→CloudKitへ別UUIDのシード世代を輸出→インポートで増殖**という消去ループになっていた（「Cloud.storeが開けない」「disk I/O error」「列欠落」「重さ」も同根）。対策: ウィジェットを**読み取り専用の消費者**に変更 — リセット機構・バージョン書き込みを全撤去し、開けなければプレースホルダ表示。両端末を `-LiminalogPurgeCloudMirrorZone` で最終一掃し、クリーン状態から再出発（storeErr/fatal 0）。**教訓: ストアの寿命管理は本体プロセスのみが持つ。拡張に削除権限を持たせない**。残課題（AI_TASKSタスク化）: シードの決定的UUID化（再シード時も同一IDになり統合不要になる堅牢化）。 |
| 2026-06-11 | Claude | データ増殖の継続報告への追加対応。原因の本丸: ①開発リセットのたびに別UUIDのシード世代がCloudKit私有DBへ蓄積しており、インポートで何世代分も流れ込み続けていた ②統合が起動時にしか走らず、セッション中の流入が次回起動まで残って見えた。対策: ①`-LiminalogPurgeCloudMirrorZone` 起動フラグ新設（DEBUG・CloudKitミラーゾーン `com.apple.coredata.cloudkit.zone` を丸ごと削除+ローカルリセット兼用）→ 両実機で実行し世代の山を一掃 ②`RootTabView` が `.NSPersistentStoreRemoteChange`（3秒デバウンス）で `CloudDuplicateMergeStore.mergeAll()` を実行 — インポート到着のたびに統合され、セッション中も増殖しない。検証: 両実機で purge+reset 成功（storeErr/fatal 0）、シミュレータ**256件全パス**。 |
| 2026-06-11 | Claude | 実機E2E検証で観測した**SwiftDataモデル無効化クラッシュ2系統**を修正（同期が本稼働した結果、await中のCloudKitインポート/重複統合がモデルを無効化する）。①LiveActivity: `LiveActivityChapterSnapshot`（プレーン値）を新設し、Chapter/CategoryをTask境界に持ち込まない。生存チェック付きinit。Intent側2箇所も追従。②友達共有の公開フロー: `prepareOutgoingShare` で「都度フェッチ→同期ブロック内でスナップショット+アイテムをプレーン値化」し、await跨ぎはIDと値型のみに（chapters/planBlocks/friendを跨いで保持しない）。③applier/applyZoneChangesに生存ガード。教訓: **awaitを跨いでSwiftDataモデルを保持しない**。あわせて、クラッシュ中の書き込みで壊れたiPhone16のストアを開発リセットで復旧（両端末: storeErr 0 / fatal 0 / fallback 0）。検証: シミュレータ**256件全パス**・両実機で安定起動。 |
| 2026-06-11 | Claude | CloudKit同期の有効化（inverse修正）で顕在化した**データ増殖**への対応。原因: 各端末が独立にシードしたデフォルト（カテゴリ/セット/プリセット/設定）+過去のミラーリング残骸が、同期復活で相互インポートされ合流。対策: `CloudDuplicateMergeStore` 新設 — 決定的勝者ルール（ユーザーID確定済み/accepted優先 → createdAt最古 → id最小。全端末が同じ勝者を選ぶ）で UserSettings(シングルトン化)/Friend(userRecordID同一視+行キャッシュ・観客スナップショット・友達セットの付け替え)/Category(実績・予定・スロット・マッピング付け替え)/CategorySet/FriendSet を統合。敗者削除はミラーリングで全端末へ伝播し収束。bootstrap先頭+シーンactive時に冪等実行。テスト5件追加。注意: UUID配列への contains は #Predicate 未対応（クラッシュ）のためメモリ内フィルタ。検証: `xcodebuild test` **256件全パス**。 |
| 2026-06-11 | Claude | 実機2台（iPhone 13/16）をMacから遠隔検証（devicectl: install/launch/console収集+実機テスト実行）。**重大バグ発見→修正**: ①`FriendCategoryMapping.friend` の inverse 欠落で CloudKit同期コンテナが実機で一度もロードできず常にローカルフォールバックだった（起動時告知アラートが可視化）→ `Friend.categoryMappings` inverse 追加で解消、**両端末でCloudKit同期コンテナが初めて正常ロード**。②CloudKit同期モデルのプロパティ削除は禁止と判明 → `sharedPlansJSON` 系を未使用残置に変更。③ステージ0の migrationPlan はモデル進化のたびロード拒否を起こすため配線解除（リリース時に再配線）。④ライブスモークテスト追加: `testSharedItemRecordsRoundTripOnRealCloudKit` が実CloudKitでparent参照付きSharedPlan/SharedChapterの保存・削除・スキーマ自動作成を確認しパス。既存スモークのダミーIDを20文字制限に追従。⑤同期成功経路に診断ログ追加（公開/受信件数）。検証: 実機ライブテスト2件パス、シミュレータ全251件パス。詳細=docs/20 §8.3-8.4。**残り**: 両端末でユーザーID再確定（ストアリセットのため・手動）→2台E2E（差分同期・トークン失効・初回接続）。 |
| 2026-06-11 | Claude | 実機検証で観測した CloudKit `zoneBusy`（"Failed to provide read-consistent view" / Retry after 2.0s）への対応。`CloudKitTransientRetryPolicy` 新設: zoneBusy/serviceUnavailable/requestRateLimited を `CKErrorRetryAfterKey`（サーバー指定の待ち時間、無指定なら指数バックオフ・上限30秒・最大3試行）でリトライ。適用箇所=ゾーン差分fetch・アイテムチャンク送信/削除・共有ルートupsert。`CloudFriendShareRefreshFailurePolicy` は zoneBusy で共有キャッシュを消さないことを確認済み（実害は同期遅延のみだった）。テスト5件追加。検証: `xcodebuild test` **251件全パス**。 |
| 2026-06-11 | Claude | 友達共有スケーラブル化（docs/20）の段階3完了=「全履歴×軽量」の輸送層（Codexトークン切れのため代行）。①個別レコード化: SharedPlan/SharedChapter を `parent`=共有ルート参照で1件1レコード化（`FriendSharedItemRecordPolicy`）。②送信差分: 内容SHA256指紋の公開台帳 `FriendSharePublishedItem`＋`FriendSharePublishDiffPolicy` で upsert/delete のみ送信（300件チャンク・非アトミック・成功分のみ台帳反映・ルート再作成時は全量再公開）。③受信差分: `CKFetchRecordZoneChangesOperation`＋ゾーントークン（`FriendShareZoneSyncState`）。失効→全件取得→全量突合。初回は共有URL承認→全件。④塊JSON完全撤去: ルートのJSONフィールド・`Friend.sharedPlansJSON`系・`FriendShareSnapshotCache`・backfill・シード塊書き込みを削除し、スナップショットをステータス専用化。期間ウィンドウ（過去365日/2000件上限）も撤廃。⑤dev store version bump（2026061101）。検証: `xcodebuild test` **246件全パス**（CKRecord往復・差分・台帳テスト追加）、シミュレータ起動+シード確認、iOS実機向け build-for-testing 成功。**⚠️ CloudKit実環境（二台実機）での検証は未実施** — docs/20 §8.3 の5項目を友達リリース前に必ず確認。 |
| 2026-06-10 | Claude | 友達共有スケーラブル化（docs/20）の段階2: UIの範囲クエリ移行完了。塊JSONの全件デコード＋全件フィルタをやめ、表示範囲だけ行キャッシュから引く形へ。①メインカレンダーの友達オーバーレイ: グリッド範囲クエリ＋初回backfill ②FriendCalendarView: 月グリッド範囲クエリ＋.taskでbackfill ③友達デイビュー: 当日範囲クエリ ④検索シート: searchPlans/hasAnyPlans ⑤後方互換 backfillFromBlobIfNeeded＋デバッグシードの行併記。テスト2件追加。検証: `xcodebuild test` **238件全パス**。※シミュレータでの操作確認はMac側の入力遮断（通知センター不可視ウィンドウ）で未実施のため、友達タブ→友達詳細→カレンダー/デイ/検索の目視確認を推奨。 |
| 2026-06-10 | Claude | 友達共有スケーラブル化（docs/20）の段階1: 受信側の個別行キャッシュ土台＋二重書き。①`FriendSharedPlanRecord`/`FriendSharedChapterRecord` 新設（LocalCacheストア=CloudKit同期なし、`#Index(friendID,startTime)`）— §2.2の「派生データを二重同期させない」問題は既存LocalCache構成で解決と決着。②`FriendSharedRecordReconcilePolicy`（sourceID突合の純ロジック）＋`FriendSharedRecordStore`（reconcile/範囲クエリ/deleteAll/purge）。③`CloudFriendShareSnapshotApplier` が塊JSONと並行して個別行へ二重書き、`clearCachedShare`で行削除、リフレッシュ末尾で孤児purge。④スキーマ4箇所（localCacheSchema/schema/V1/TestModelContainer）へ追加。⑤テスト7件追加。docs/20へ§7（実装状況・残りの着手順）を追記し、UIの範囲クエリ移行=Claude / CloudKit輸送層=Codex のタスクを§7.3に登録。検証: `xcodebuild test` **236件全パス**。 |
| 2026-06-10 | Claude | UIレビュー指摘の磨き込み4件＋文字サイズ固定。①統計ヒーローカードのスパークラインに日付ラベル追加（`ScoreTrendBarChart` に `showsDateLabels`。週間=曜日1文字、年間=月、長い日次系列=5日刻み）。②設定入口は一度カレンダーの歯車へ統一したが、ユーザー確認の結果**従来どおりに復元**: プロフィール右上ハンバーガー=アプリ全体設定、カレンダーの歯車=カレンダー表示設定（`CalendarSettingsSheet`）。歯車のアクセシビリティラベルだけ「カレンダー表示設定」へ明確化。③下タブ「今日」→「ホーム」に改名。④コレクションギャラリーを2列→3列のコンパクトタイル化。装着ボタンをタイルから外し、タップで詳細シート（拡大プレビュー・解放条件・進捗・控えめな装着ボタン、medium detent）。装着の主導線は将来プロフィールカード編集側へ。⑤`RootTabView` に `.dynamicTypeSize(.large)` を追加し、システム文字サイズ設定の影響を遮断（ユーザー判断によるサイズ固定）。検証: シミュレータで全画面目視確認、`xcodebuild test` 229件全パス。※調査中、テストランナーの「hung before establishing connection」が頻発したが原因はCoreSimulator側の不調（コミット済み状態・変更後状態の双方で再現/解消し、最終的に全構成でパス）。また開発シミュレータで `Cloud.store` の `ZAUDIENCEFRIENDIDS` 列欠落（古いウィジェットバイナリによるスキーマダウングレードと推定）に遭遇 → `-LiminalogResetDevelopmentStore` で復旧。 |
...
| 2026-06-02 | Codex | ユーザー指摘を受け、カレンダー月グリッドの予定ラベル描画を「通常予定は不透明、過去予定dim設定時だけ半透明」へ整理。背景付き予定ラベル/複数日バーは `color.opacity(...)` ではなく、予定色をsystem backgroundへブレンドした不透明色で塗るようにし、背景付き表示の文字色は `.primary` / `.secondary` に戻して、背景色と文字色が同系で読めなくなる問題を避けた。下線表示も不透明なブレンド色へ変更。検証: `xcodebuild -project Liminalog.xcodeproj -scheme Liminalog -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /private/tmp/LiminalogDerivedData build` 成功、Simulator iPhone 17 Proでlight/dark appearanceのカレンダーを撮影し、背景付き予定の文字が色面に沈まないことを確認。スクショ: `/private/tmp/liminalog-daybreak-calendar-plan-solid-text.png`, `/private/tmp/liminalog-dusk-calendar-plan-solid-text.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー指摘を受け、ライトモードのカレンダー月グリッドでも予定色を見やすく調整。前回のダーク専用強化に続き、ライト時の予定ラベル/複数日バーの塗り・枠線・下線・文字色を少し強め、白背景上でも赤/紫/青/橙の予定が淡く溶けないようにした。ダーク時の値は維持。検証: `xcodebuild -project Liminalog.xcodeproj -scheme Liminalog -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /private/tmp/LiminalogDerivedData build` 成功、Simulator iPhone 17 Proでlight appearanceのカレンダーを撮影し、予定バーが白背景から判別できることを確認。スクショ: `/private/tmp/liminalog-daybreak-calendar-plan-color-clear.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー指摘を受け、ダークモードのカレンダー月グリッドで予定色が薄く見える問題を調整。`CalendarImportantPlanLabel` / `CalendarMultiDayPlanBar` に `colorScheme` 分岐を追加し、ダーク時だけ予定ラベル/複数日バーの塗り・枠線・下線・文字色を強めた。ライト時の白背景向け控えめ表示は維持。検証: `git diff --check` 成功、Simulator iPhone 17 Proでdark appearanceのカレンダーを撮影し、赤/紫/青/橙の予定が月グリッド上で沈まず読めることを確認。スクショ: `/private/tmp/liminalog-dusk-calendar-plan-color-strong.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー指摘を受け、ダークモードのカレンダー月グリッドの日付区切りを強化。`CalendarMonthGrid` のセル間divider/外枠を `colorScheme` で分岐し、ダーク時は白ベースの低不透明度ラインへ変更して、セル面と区切りが同化しないようにした。ライト側の既存 separator 値は維持。検証: `git diff --check` 成功、`xcodebuild -project Liminalog.xcodeproj -scheme Liminalog -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /private/tmp/LiminalogDerivedData build` 成功。Simulator iPhone 17 Proをdark appearanceにしてカレンダータブを撮影し、日付セルの縦横区切りが見えることを確認。スクショ: `/private/tmp/liminalog-dusk-calendar-date-dividers.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー指摘を受け、Todayタブ内タブの左右スワイプを復旧。前回の「初期表示を今日にする」修正で横ページングの物理順を初期ページ起点へ並べ替えていたため、通常起動時に `[今日, 昨日, 明日]` となり左右の連続移動が崩れていた。`HomeView` の `scrollPages` を正順 `[昨日, 今日, 明日]` に戻し、初期ページだけをレイアウト後に複数回再適用してからスクロール同期を開くようにした。検証: `git diff --check` 成功、`xcodebuild -project Liminalog.xcodeproj -scheme Liminalog -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -derivedDataPath /private/tmp/LiminalogDerivedData build` 成功。Simulator iPhone 17 Proで通常起動が「今日」、横スワイプで「明日」、逆方向スワイプ2回で「昨日」へ移動できることを確認。スクショ: `/private/tmp/liminalog-today-swipe-fix-initial.png`, `/private/tmp/liminalog-today-swipe-fix-after-left-2.png`, `/private/tmp/liminalog-today-swipe-fix-after-right-2.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | Phase 4 先行で Siri ショートカット/App Intents を拡充。`LiminalogAppShortcutsProvider` に「今日」「カレンダー」「統計」「プロフィール」を開く4つの `Open*ShortcutIntent` と、現在のactive Chapterを閉じる `EndChapterIntent` を登録した。画面を開くIntentは App Group UserDefaults の `shortcut.pendingRoute` に予約し、`RootTabView` が起動時/active復帰時に1回だけ消費して目的タブへ移動する。`EndChapterIntent` はactive Chapterを全収束して終了し、activeカテゴリcache、RecordingGridWidget、Live Activityを保存後状態へ更新する。カテゴリ指定の `StartChapterIntent` はWidget/Live Activity向けとして残す。検証: `xcodebuild test -scheme Liminalog -destination 'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.5' -derivedDataPath /private/tmp/LiminalogDerivedData -only-testing:LiminalogTests/LiminalogShortcutRouteTests` 成功（1 test）。AppIntents metadata trainingログに5ショートカットが出力されたことを確認。 |
...
| 2026-06-02 | Codex | 装飾アイテムモデル設計を docs/09 の8分類へ整理。`UnlockKind` から暫定の `appIcon` / `stamp` / `cardTemplate` を外し、`iconSet` / `monthArt` を追加して、フレーム/バッジ/炎/アイコンセット/テーマ/バー/カード/月アートの8種類を型と seed の両方で表現した。`UnlockCatalog` は26件の解放スケジュールを維持しつつ iconSet/monthArt seed を含め、既存開発ストアの旧 built-in key は `UnlockStore.seedMasterItems` で新keyへ移行し `unlockedAt` を保持する。docs/04 と `UnlockRulesTests` も更新。検証: `git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功。対象テスト `UnlockRulesTests` / `ProfileDecorationUnlocksTests` はコンパイル完了後に Simulator 実行フェーズが無出力で止まったため中断し、通過扱いにはしていない。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | StatsEngine / PatternDetector 共通化を完了。`DailyCardPatternDetector` を Home 専用ファイルから Logic 層へ移動し、`DailyCardPatternSignal` として検出結果を共有可能にした。`StatsEngine` は DailyCard 用パターン検出、Dashboard 用の日別 `ScoreSummary`、時間帯別傾向、前期間差分を返す共通入口になり、`DailyCardEngine` は称号/本文/fact のコピー変換だけを持つ構成へ整理。検証: `git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功。対象テスト `StatsEngineTests` / `DailyCardEngineTests` / `DashboardAnalyticsTests` は一度テストコンパイルエラーを修正後、再実行でビルドまでは進んだが Simulator 実行フェーズが無出力で止まったため中断し、通過扱いにはしていない。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー指摘を受け、Todayタブ内タブの初期表示が「昨日」に寄る問題を修正。`HomeView` の横ページング初期化で、レイアウト直後に `scrollPosition` が先頭ページを返して `selectedPage` を上書きしないよう、初期ページ適用中はスクロール同期を一時停止し、次runloopで `debugInitialTodayPage`（通常は今日、DEBUG引数指定時のみ任意ページ）を再適用する。検証: `git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功、Simulator iPhone 17 Proへ通常起動（`-LiminalogInitialTodayPage` なし）して上部タブが「今日」選択で開くことを確認。スクショ: `/private/tmp/liminalog-today-default-tab.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | P2-4 自己説明ラベル削除を完了。`DailyReflectionCard` の表示ピル「予定と実績/実績のみ」はアイコンのみへ降格し、`CalendarDayView` の日別スコア見出し「予定と実績」と友達日詳細サブタイトル「共有された予定と実績」を削除。状態説明は必要箇所のみ `accessibilityLabel` に残し、表面のchromeを減らした。検証: `rg` で表示 `Text` の「24時間バー」「予定と実績」「実績のみ」「共有された予定と実績」が自己説明ラベルとして残っていないことを確認（共有画像のフッターコピー「予定と実績のあいだを、記録する。」のみ意図して維持）、`git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー指摘を受け、UIコピーから「重なり」系の見出しを撤去。`HomeView` の昨日カードサブタイトルは「予定と実績を振り返ります」、`CalendarDayView` の日別スコア見出しは「予定と実績」に変更し、「予定との重なり」が画面に出ないようにした。検証: Swift UI 表示文字列の `rg` で「重なり」系コピーなし、`git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー指摘を受け、曙の `DailyReflectionCard` リングライトを再調整。白に近い下地では黄色いライトが強く見えるため、ライト時の `DailyTwentyFourHourRing` 中央グローから `reward`/ピーチ系を外し、低不透明度の `dusk` + `primary` + ほぼ白ラベンダーへ変更。曙だけグロー半径/blurも縮小し、中央アイコンのライトも弱めた。宵は従来の紫グロー値を維持。検証: `git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功、Simulator iPhone 17 Proで署名あり `xcodebuild ... build` 成功。スクショ: `/private/tmp/liminalog-daybreak-yesterday-ring-center-no-label.png`, `/private/tmp/liminalog-dusk-yesterday-ring-center-no-label.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー確認を受け、7.7.2 G のリング中央整理を撤回。`DailyTwentyFourHourRing` は `d14f979` の小バッジ退避/スコア主役構成から、`0bc543f` 寄りの中央大アイコン + `99pt` 構成へ戻しつつ、「予定との重なり」説明ラベルは削除した。曙の黄色グロー抑制は維持し、中央アイコンのlight時ライトだけ弱めて白地での違和感を抑えた。検証は同上の build-for-testing / Simulator light-dark スクショで実施。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | 7.7.2 G リング中央の整理を完了。`DailyTwentyFourHourRing` の中央から大きなペルソナグリフ玉を外し、32ptの小バッジとして内リング上側へ退避。中央は `99` を主役に、`pt` とラベルを小さくして docs/13 §5.3 の「余白基調」に寄せた。検証: `git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功、Simulator iPhone 17 Proで署名あり `xcodebuild ... build` 成功。スクショ: `/private/tmp/liminalog-daybreak-yesterday-center-air.png`, `/private/tmp/liminalog-dusk-yesterday-center-air.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | ユーザー指摘を受け、曙の `DailyReflectionCard` リング中央ライトを調整。ライト側だけ `DailyTwentyFourHourRing` の中央放射グローを紫ベースから `reward` + ピーチ + ごく薄い `dawn` の暖色ベースへ変更し、暗めの紫でくすんで見える印象を抜いた。宵は従来どおり `primary` の紫グローを維持し、曙の薄ラベンダーは外枠/チップ側に残した。検証: `git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功、Simulator iPhone 17 Proで署名あり `xcodebuild ... build` 成功。スクショ: `/private/tmp/liminalog-daybreak-yesterday-warm-ring-center.png`, `/private/tmp/liminalog-dusk-yesterday-warm-ring-center-regression.png`。CSV書き出しは不要・スコープ外を維持。 |
| 2026-06-02 | Codex | P2-7 昨日ページ重複ブロック撤去を完了。`YesterdayReviewPage` の `DailyReflectionCard` 下に残っていた旧 `YesterdayCategoryBreakdown` / `YesterdayInsightCard` を削除し、関連する未使用View群も撤去してカードを主役に整理。あわせてユーザー指摘を受け、今日ページ/友達読み取り専用タイムラインの inline toggle を中央寄せ + 余白広めに調整し、元トグルに近い重心へ戻した。検証: `git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功、Simulator iPhone 17 Proで署名あり `xcodebuild ... build` 成功。スクショ: `/private/tmp/liminalog-daybreak-timeline-inline-centered.png`, `/private/tmp/liminalog-dusk-timeline-inline-centered.png`, `/private/tmp/liminalog-daybreak-yesterday-p2-7.png`, `/private/tmp/liminalog-dusk-yesterday-p2-7.png`。light/darkともトグルの中央配置、昨日カードの文字消えなし、旧カテゴリ/インサイトブロックが主面に残っていないことを確認。CSV書き出しは不要・スコープ外を維持。 |
...
| 2026-06-02 | Codex | P3-9 友達タブIA調整を完了。`FriendsView` でランキングを、友達数が増えても長くならない固定サマリーとして上部に配置し、ランキングカードは幅・余白・スコア文字・面/枠線の強さを落として競争を従にした。「今の友達」リストはその下で、友達行は @handle を主情報に出さず、名前・現在ステータス・ひとことを主役に維持。検証: `git diff --check` 成功、`xcodebuild -scheme Liminalog -destination generic/platform=iOS -derivedDataPath /private/tmp/LiminalogDerivedData CODE_SIGNING_ALLOWED=NO build-for-testing` 成功、Simulator iPhone 17 Proで署名あり `xcodebuild ... build` 成功。`/private/tmp/liminalog-daybreak-friends-ranking-top.png` と `/private/tmp/liminalog-dusk-friends-ranking-top.png` を撮影し、light/darkともランキングが上部固定サマリーとして収まり、リストの現在ステータス/ひとことも読めることを確認。CSV書き出しは不要・スコープ外を維持。 |
...
- [x] **DashboardView(1192行) / ProfileView(1058行) のサブビュー外出し**（担当: Codex, 完了: 2026-06-02）
  - FriendsView ほどではない（単一タブ内で凝集・他タブの複製ではない）が、ナビゲーション性のためファイル分割したい。優先度は上記2件より低。
  - 2026-06-02 Codex完了: `DashboardView.swift` からカード表示部品を `DashboardCards.swift`、集計/フォーマット型を `DashboardModels.swift` へ外出し。`ProfileView.swift` から表示部品を `ProfileComponents.swift`、編集シートを `ProfileEditSheet.swift`、装飾カタログを `ProfileDecorations.swift`、集計スナップショットを `ProfilePerformanceSnapshot.swift` へ分離。画面本体は Dashboard 400行、Profile 192行まで縮小。
