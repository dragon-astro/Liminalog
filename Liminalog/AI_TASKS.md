...
- [ ] 友達共有スケーラブル化: UIを個別行の範囲クエリへ移行（docs/20 §7.3-1。FriendCalendarView/友達デイビューの塊デコードを `FriendSharedRecordStore.plans/chapters(friendID:overlapping:)` に置換、`FriendShareSnapshotCache` 縮小） <!-- 担当: Claude, 理由: 既存SwiftUIの整合性維持。土台と二重書きは実装済み（2026-06-10） -->
